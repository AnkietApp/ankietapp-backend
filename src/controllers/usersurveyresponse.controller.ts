...
export const updateSurveyResponse = async (
  ...
  try {
    ...
    const updatedResponse = await getRepository(UserSurveyResponse).findOne({
      where: { id: req.params.id, userId: user.id, completed: true }
    });
    console.log(updatedResponse);

    req.body.answers.forEach(async (answer: Answer) => {
      const newAnswer: Answer = getRepository(Answer).create({
        ...answer,
        userSurveyResponseId: updatedResponse!.id
      });
